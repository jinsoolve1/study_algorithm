## [백준 2465 - 줄 세우기](https://www.acmicpc.net/problem/2465)
...
### 풀이
수열 S를 뒤에서부터 보는 것이 핵심이다.  
맨 뒤부터 ans[N] = S[N]번째 키가 된다. (ans[N-1] = S[N-1]번째 키 ... )  
뽑은 키는 이제 없는 키로 취급하고 다음 값들을 찾는다. 이때 없는 값은 segment tree로 구현하면 된다.  
세그먼트 트리의 리프노드를 모두 1로 초기화하고 사용하고나면 0으로 바꿔주면 된다.  

ans[i] 를 찾을 때 S[i]번째 키를 찾는 법은 세그먼트 트리에서 이분탐색으로 찾으면 된다.
