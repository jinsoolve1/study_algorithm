...
### 알고리즘
- Min Cost Max Flow (유량 알고리즘) 
- 정점 분할

### 풀이
1. (사람in -> 사람out, cap=a, cost=0) 그리고 (서점in -> 서점out, cap=b, cost=0) 으로 정점분할 한다.
2. (사람out -> 서점in, cap=INF, cost=c)로 edgeㄹ르 추가한다.
3. Src -> Sink 의 mincost maxflow를 구한다.  

Src = 0, Sink = 401  
사람in = 1 ~ 100, 사람out = 101 ~ 200, 서점in = 201 ~ 300, 서점out = 301 ~ 400
